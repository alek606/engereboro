...
const vcard = 'BEGIN:VCARD\n' 
            + 'VERSION:1.0\n' 
            + 'FN:gothic angel\n' 
            + 'ORG:Dono do engere bots;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=556993966533:+55 (69) 99396-6533\n' 
            + 'END:VCARD'
...
async function starts() {
	...
	tiringa.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar o engere bots!'))
	})

	fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
	tiringa.on('connecting', () => {
		start('2', 'Conectando o engere bots...')
	})
	tiringa.on('open', () => {
		success('2', 'engere bots conectado!!!')
	})
...
tiringa.updatePresence(from, Presence.composing)
reply("Oi amor da minha vida🥰")
}
if ((budy === "yumi gay") || (budy === "Yumi gay")) {
tiringa.updatePresence(from, Presence.composing)
reply("sim linda vdd")
}
...
if ((budy.includes('kkk')) || (budy.includes('KKK'))) {
reply('Qual a graça?🤨')
}
...
tiringa.sendMessage(from, `VITÓRIA DO ENGERE BOTS \n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
} else {
tiringa.sendMessage(from, `VITÓRIA DO ENGERE BOTS`, text)
}
...
yhb = `Que fofo... @${sender.split("@")[0]} deu um abraço apertado e passou a pika no @${mentioned[0].split('@')[0]}`
...
yhb =  `@${mentioned[0].split('@')[0]} tem uma chance de ${porc}% de namorar com @${mentioned[1].split('@')[0]} ala o web cornokkkkjkkkjkk`,
...
tiringa.sendMessage(from, buffer, image, {quoted: mek, caption: '🚔 ó a puliça 🚔'})
})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('❌ocorreu um erro❌\n\nTente novamente. ')
...
tiringa.sendMessage(from, mp4, MessageType.video, {mimetype: 'video/gif', filename: `stick.gif`, quoted: mek, caption: 'eita'})
})
})
} catch (e) {
console.log(`Error :`, color(e,'red'))
